...
Set GOOGLE_CLOUD_PROJECT and PARALLEL_API_KEY in .env.local, then open http://localhost:3000.
...
Grant the Cloud Run runtime service account roles/aiplatform.user, roles/datastore.user, and Secret Manager access to parallel-api-key. The /api/health endpoint reports configuration without exposing credentials.

The supplied public hackathon deployment uses fictional production records. Before storing real data, put the service behind Identity-Aware Proxy or Firebase Authentication and enforce production membership. Firestore rules are deny-by-default; server writes use IAM.
